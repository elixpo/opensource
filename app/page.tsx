...
export default function Home() {
  return (
    <main>
      ...
      <div className="overflow-hidden border-y border-[var(--line)] bg-cream py-4"><div className="ticker flex w-max items-center gap-12 whitespace-nowrap font-mono text-[11px] font-bold uppercase tracking-[.12em] text-[#777]">{[...Array(2)].flatMap(() => ['Programs', 'Contributors', 'Mentors', 'Repositories', 'Leaderboards', 'Payouts']).map((item, i) => <span key={`${item}-${i}`} className="flex items-center gap-12">{item}<b className="text-accent">✦</b></span>)}</div></div>
