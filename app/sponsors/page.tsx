					...
					<div className="shell">
						...
						<div className="mt-12 flex flex-col gap-8">
							{tiers.map((tier) => (
								<div
									key={tier.name}
									className="surface p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-8 hover:border-[#bbb] transition"
								>
									...
									<div>
										<div className="flex flex-wrap gap-3">
											{tier.sponsors.map((sponsor) => (
												<span
													key={sponsor}
													className="rounded-lg border border-[var(--line)] bg-[#fafafa] px-5 py-3 text-sm font-black text-ink shadow-sm"
												>
													{sponsor}
												</span>
											))}
